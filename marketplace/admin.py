...
from .models import Agency, Package, Offer, Booking, ChatRoom, Message, PaymentRequest, AgencyWarning, TripUpdate, TripStatus
...
class BookingAdmin(admin.ModelAdmin):
    list_display  = ('id', 'user', 'destination', 'agency', 'base_amount', 'gst_amount', 'total_amount', 'is_paid', 'created_at')
    list_filter   = ('is_paid',)
    search_fields = ('user__username', 'offer__trip__destination', 'offer__agency__name')
...
class ChatRoomAdmin(admin.ModelAdmin):
    list_display  = ('id', 'user', 'agency', 'destination', 'is_active', 'created_at')
    list_filter   = ('is_active',)
    search_fields = ('user__username', 'agency__name', 'offer__trip__destination')
    ordering      = ('-created_at',)

    def destination(self, obj):
        return obj.offer.trip.destination
    destination.short_description = 'Trip'
...
class TripStatusAdmin(admin.ModelAdmin):
    list_display  = ('booking_dest', 'traveler', 'agency', 'status', 'updated_at')
    list_filter   = ('status',)
    search_fields = ('booking__offer__trip__destination', 'booking__user__username')
    ordering      = ('-updated_at',)

    def booking_dest(self, obj):
        return obj.booking.offer.trip.destination
    booking_dest.short_description = 'Destination'

    def traveler(self, obj):
        return obj.booking.user.username
    traveler.short_description = 'Traveler'
